...
(async () => {
    for (const keyword of keywordsJobs) {
        ...
        while (true) {
            ...
            if (jobCount >= 60) {
                console.log(`Se han obtenido 60 trabajos para la keyword "${keyword}", finalizando el proceso.`);
                break;  // Rompe el bucle while si se llega a 32 trabajos
            }
            ...
            try {
                ...
                for (let i = 0; i < Math.min(companyElements.length, titleElements.length, locationElements.length, dateElements.length); i++) {
                    try {
                        ...
                        jobCount++;  // Incrementa el contador por cada trabajo procesado

                        // Si se alcanzan los 32 trabajos, sal del bucle
                        if (jobCount >= 60) {
                            console.log(`Se han obtenido 32 trabajos para la keyword "${keyword}", finalizando el proceso.`);
                            break;
                        }
